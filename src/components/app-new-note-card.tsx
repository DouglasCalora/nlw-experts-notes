...
export interface AppNewNoteCardProps {
...
function Content () {
  return (
      <p className="text-slate-400 left-6 font-medium">
        Comece <button type="button" className="text-lime-400" onClick={handleStartRecording}>gravando uma nota</button> em áudio ou se preferir <button type="button" className="text-lime-400" onClick={handleStartEditor}>utilize apenas texto</button>.
      </p>
  )
}
...
function DefaultButton (props: { saveNote: () => void }) {
  return (
      <button
...
export default function NewNoteCard (props: AppNewNoteCardProps) {
        ...
        <Dialog.Content className="fixed z-10 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full bg-slate-700 h-[60vh] rounded-md flex flex-col outline-none overflow-hidden">
          ...
          <form className="flex flex-col flex-1">
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {shouldShowOnboarding ? Content() : Textarea()}
            </div>

            {isRecording ? RecordingButton() : DefaultButton()}
          </form>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
